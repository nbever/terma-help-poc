...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:choose>
      <xsl:when test="@href">
        <a>
          ...
          <xsl:call-template name="commonAttributes"/>
          <xsl:call-template name="externalLinkAttributes"/>
  ...
  <xsl:template name="processFig">
    ...
    <xsl:choose>
      <xsl:when test="$title">
        ...
        <xsl:choose>
          <xsl:when test="index-of($titleAfterList,$type) ge 1">
  ...
  <xsl:template name="imageToImg">
    ...
    <img src="{@href}">
      ...
      <xsl:if test="$usemap ne ''">
        <xsl:attribute name="usemap" select="$usemap"/>
      </xsl:if>
    </img>
  </xsl:template>
  ...
  <xsl:template name="processAudioVideoParams">
    ...
    <xsl:for-each select="$params">
      ...
      <xsl:if test="$paramName ne '' and $paramName ne 'source'">
        <xsl:choose>
    ...
    <source src="{normalize-space(@data)}" type="{normalize-space(@type)}"/>

    <!-- Alternate sources -->

    <xsl:for-each select="$params">
      <xsl:variable name="name" select="string(@name)"/>
      <xsl:variable name="src" select="normalize-space(@value)"/>

      <xsl:if test="$name eq 'source' and $src ne ''">
        <source src="{$src}">
      ...
      <xsl:otherwise>
        <img>
          ...
          <xsl:if test="$media-icon-width ne ''">
            <xsl:attribute name="width" select="$media-icon-width"/>
          </xsl:if>
          <xsl:if test="$media-icon-height ne ''">
            <xsl:attribute name="height" select="$media-icon-height"/>
          </xsl:if>
        </img>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="*[contains(@class,' topic/param ')]">
    <param>
      ...
      <xsl:if test="$xhtmlVersion ne '5.0'">
