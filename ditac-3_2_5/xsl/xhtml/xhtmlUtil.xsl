...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="commonAttributes">
    ...
    <xsl:call-template name="flagsAttribute"/>
  </xsl:template>
  ...
  <xsl:template name="flagsAttribute">
    <xsl:variable name="style" select="u:flagsAttributeValue(.)"/>
    <xsl:if test="$style ne ''">
      <xsl:attribute name="style" select="$style"/>
    </xsl:if>
  </xsl:template>

  <xsl:function name="u:flagsAttributeValue" as="xs:string">
    <xsl:param name="context" as="element()"/>

    <!-- Degraded flagging mode. Only use colors and text styles. -->
    <xsl:sequence
      select="if ($context/parent::ditac:flags and u:canFlag($context) eq 0)
              then u:flagsStyle($context/parent::*, false())
              else ''"/>
  </xsl:function>
  ...
  <xsl:template name="displayAttributes">
    ...
    <xsl:if test="$style ne ''">
      <!-- No need to concat u:flagsAttributeValue(.). displayAttributes
           is invoked for elements which are fully flaggable. -->
  ...
  <xsl:template name="externalLinkAttributes">
    ...
    <xsl:if test="@scope eq 'external'">
      ...
      <xsl:variable name="format"
                    select="lower-case(normalize-space(@format))" />
      <xsl:if test="$format ne ''">
        <xsl:variable name="i" select="index-of($formatToType, $format)" />
        <xsl:variable name="type"
          select="if ($i &gt; 0) then $formatToType[$i + 1] else ''" />
        <xsl:if test="$type ne ''">
