...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:choose>
          <xsl:when test="$foProcessor eq 'XFC'">
            <!-- XFC does not support list-item/@relative-align. 
                 Borders around a table having an auto layout 
                 does not look right. -->
  ...
  <xsl:template
    match="*[contains(@class,' topic/ol ')]/*[contains(@class,' topic/li ')]  |
           *[contains(@class,' topic/ol ')]/ditac:flags/*[contains(@class,' topic/li ')]">
    <xsl:choose>
        ...
        <xsl:otherwise>
          <fo:block xsl:use-attribute-sets="ol-li-label">
            <xsl:variable name="ol"
                          select="ancestor::*[contains(@class,' topic/ol ')]"/>
            <xsl:choose>
              <xsl:when test="(number($olStart) ge 0) or
                              exists($ol/ditac:flags)">

                <xsl:variable name="liSiblings"
                  select="$ol/*[contains(@class,' topic/li ')] | 
                          $ol/ditac:flags/*[contains(@class,' topic/li ')]"/>
                ...
                <xsl:number format="{$format}" 
                            value="$start + u:indexOfNode($liSiblings, .) - 1"/>
              </xsl:when>
  ...
  <xsl:template
    match="*[contains(@class,' topic/ul ')]/*[contains(@class,' topic/li ')] |
           *[contains(@class,' topic/ul ')]/ditac:flags/*[contains(@class,' topic/li ')]">
    <xsl:choose>
      <xsl:when test="parent::*/@compact eq 'yes'">
