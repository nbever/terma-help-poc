...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="equationlistId" select="'__LOM'"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:choose>
          ...
          <xsl:when test="$role eq 'equationlist'">
            <xsl:sequence select="$equationlistId"/>
          </xsl:when>
  ...
  <xsl:function name="u:shortTitlePrefix" as="xs:string">
    ...
    <xsl:choose>
      ...
      <xsl:when test="(starts-with($lastPart, 'part.') and 
        ...
        <xsl:sequence select="u:insertNumber($label, $num, $context)"/>
      </xsl:when>
      ...
      <xsl:when test="((starts-with($lastPart, 'section1.') or 
        ...
        <xsl:variable name="first"
                      select="for $part in $parts
                              return 
                                  if (starts-with($part, 'section1.')) 
                                  then index-of($parts, $part)
                                  else ()"/>
            ...
            <xsl:variable name="from"
              select="if ($prepend-chapter-to-section-number eq 'yes' and
                          ...
                          (starts-with($parts[$first - 1], 'chapter.') or 
                           starts-with($parts[$first - 1], 'appendix.')))
                      then $first - 1
                      else $first"/>
      ...
      <!-- Format (chapter|appendix)? table|figure|example|equation "."
           Note that 'all' includes 'table', 'fig' and 'equation-figure' 
           but not 'example'.  
           'figure' alias for 'fig'. 
           'equation' alias for 'equation-figure'. -->
      <!-- equation-numbers are always numbered. -->
      <xsl:when
        test="(starts-with($lastPart, 'table.') and 
               (index-of($numberList, 'all') ge 1 or 
                index-of($numberList, 'table') ge 1))
           or
           (starts-with($lastPart, 'figure.') and 
            (index-of($numberList, 'all') ge 1 or 
             index-of($numberList, 'fig') ge 1 or 
             index-of($numberList, 'figure') ge 1))
           or
           (starts-with($lastPart, 'equation.') and 
            (index-of($numberList, 'all') ge 1 or 
             index-of($numberList, 'equation-figure') ge 1 or 
             index-of($numberList, 'equation') ge 1))
           or
           (starts-with($lastPart, 'example.') and 
            index-of($numberList, 'example') ge 1)
           or
           ($context/self::*[contains(@class,' equation-d/equation-number ')])">
        <xsl:variable name="label" 
                      select="u:localize(substring-before($lastPart, '.'), 
                                         $context)"/>

        <xsl:variable name="partRange" 
                      select="if (starts-with($parts[1], 'part.')) 
                              then remove($parts, 1) 
                              else $parts"/>
        <xsl:variable name="num"
      ...
      <xsl:when test="$prefix ne ''">
        ...
        <xsl:choose>
          <xsl:when test="starts-with($lastPart, 'section1.') or
                          ...
                          starts-with($lastPart, 'example.') or
                          starts-with($lastPart, 'equation.')">
            <!-- Prepend "Part N, Chapter M" or "Appendix L", if any. -->

            <!-- Notes:
                 * Prepending "Part N, Chapter M" or "Appendix L" to
                   a figure, table, equation or example may be a bit redundant 
                          ...
                          select="for $part in $parts
                                  return 
                                      if (starts-with($part, 'section1.') or
                                          ...
                                          starts-with($part, 'example.') or
                                          starts-with($part, 'equation.'))
                                      then index-of($parts, $part)
                                      else ()"/>
            <xsl:choose>
  ...
  <xsl:function name="u:formatSegments" as="xs:string*">
    ...
    <xsl:sequence 
      select="for $part in $parts
              return
                  u:insertNumber(u:localize(substring-before($part, '.'), 
                                            $context), 
          ...
          <xsl:when test="$role eq 'section1'">
            ...
            <xsl:choose>
              <xsl:when test="$prefix ne ''">
                ...
                <xsl:value-of 
                  select="concat(u:extractNumber($prefix, $tocEntry),
  ...
  <xsl:template name="titlePrefix">
    ...
    <xsl:if test="$id ne ''">
      <xsl:choose>
        ...
        <!-- Before figure processing.
             A "equation-d/equation-figure" is also a "topic/fig". -->
        <xsl:when
          test="contains($parent/@class,' equation-d/equation-figure ')">
          <xsl:variable name="equation" 
            select="$ditacLists/ditac:equationList/ditac:equation[@id eq $id]"/>
          <xsl:if test="exists($equation)">
            <xsl:variable name="prefix" 
                          select="u:shortTitlePrefix(string($equation/@number), 
                                                     $equation)"/>
            <xsl:if test="$prefix ne ''">
              <xsl:value-of select="concat($prefix,
                                           $title-prefix-separator1)"/>
            </xsl:if>
            <!-- Otherwise, equation are not numbered. -->
          </xsl:if>
          <!-- Equations having no title are not added to equationList. -->
        </xsl:when>
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:variable name="equation" 
            select="$ditacLists/ditac:equationList/ditac:equation[@id eq $id]"/>
          <xsl:choose>
            <xsl:when test="exists($equation)">
              <xsl:variable name="prefix" 
                select="u:relativeTitlePrefix(string($equation/@number),
                                              $topicNumber,
                                              $equation)"/>
              <xsl:if test="$prefix ne ''">
                <xsl:value-of select="concat($prefix,
