...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- ditac:toc,figureList,tableList,exampleList,equationList,indexList ==-->
  <!-- All suppressed. -->
  ...
  <!-- Workaround a limitation of the JavaHelp viewer:
       Requires none or both width and height to be specified. -->

  <xsl:template name="imageSizeAttributes">
    <xsl:if test="exists(@width) or exists(@height) or exists(@scale)">
      <xsl:variable name="imageW" as="xs:double">
        <xsl:call-template name="imageWidth">
          <xsl:with-param name="path" select="@href"/>
        </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="imageH" as="xs:double">
      ...
      <xsl:choose>
        <xsl:when test="exists(@width) or exists(@height)">
          <xsl:if test="exists(@width)">
            <xsl:variable name="width" select="u:toPixels(string(@width))"/>
            <xsl:attribute name="width" select="$width"/>

            <xsl:if test="empty(@height) and $imageW gt 0 and $imageH gt 0">
              <xsl:attribute name="height" 
                select="string(round((number($width) * $imageH) div $imageW))"/>
            </xsl:if>
          </xsl:if>

          <xsl:if test="exists(@height)">
            <xsl:variable name="height" select="u:toPixels(string(@height))"/>
            <xsl:attribute name="height" select="$height"/>

            <xsl:if test="empty(@width) and $imageW gt 0 and $imageH gt 0">
              <xsl:attribute name="width"
               select="string(round((number($height) * $imageW) div $imageH))"/>
            </xsl:if>
          </xsl:if>
        </xsl:when>

        <xsl:when test="number(@scale) gt 0 and $imageW gt 0 and $imageH gt 0">
          <xsl:attribute name="width"
            select="string(round(($imageW * number(@scale)) div 100.0))"/>
          <xsl:attribute name="height"
            select="string(round(($imageH * number(@scale)) div 100.0))"/>
        </xsl:when>

        <!-- @scalefit eq 'yes' not supported. -->
      </xsl:choose>
    </xsl:if>
  </xsl:template>
