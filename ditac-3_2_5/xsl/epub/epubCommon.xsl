...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="processMathML">
    <xsl:choose>
      <xsl:when test="$xhtmlVersion eq '5.0' and $epub2-compatible ne 'yes'">
        <xsl:apply-templates select="." mode="mathml"/>
      </xsl:when>
      <xsl:otherwise>
        <epub:switch xmlns:epub="http://www.idpf.org/2007/ops">
          <epub:case required-namespace="http://www.w3.org/1998/Math/MathML">
            <!-- Force use of http://www.w3.org/1998/Math/MathML as the
                 default namespace. -->
            <xsl:apply-templates select="." mode="mathml"/>
          </epub:case>
          <!-- Firefox EPUB reader displays what's inside the default. -->
  ...
  <xsl:function name="u:mediaType">
    ...
    <xsl:sequence
      select="if (ends-with($path2, '.gif')) then 'image/gif'
              ...
              else if (ends-with($path2, '.mp4') or
                       ends-with($path2, '.m4v')) then 'video/mp4'
  ...
  <xsl:template name="appendObject">
    ...
    <xsl:for-each select="./*[contains(@class,' topic/param ')]">
      ...
      <xsl:if test="($name eq 'source' or $valuetype eq 'ref') and $src ne ''">
