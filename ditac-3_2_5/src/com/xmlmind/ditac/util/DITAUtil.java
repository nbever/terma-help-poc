...
public final class DITAUtil implements Constants {
    private DITAUtil() {}
    ...
    private static String[] getMetaAttributes(Element element, 
        ...
        if (domains != null) {
            ...
            for (;;) {
                ...
                for (String name : names) {
                    if (!StringList.contains(allAttrNames, name)) {
        ...
        /*
        System.err.println(specializationPrefix + " ---> " + 
                           StringUtil.join(' ', allAttrNames));
        */
    ...
    public static URL doGetLocalTopicURL(Element topicref) 
        ...
        String scope = DITAUtil.getScope(topicref, href);
    ...
    /**
     * Return the <tt>scope</tt> of specified <tt>topicref</tt>. 
     * <em>Never</em> returns <code>null</code>; fallbacks to "<tt>local</tt>".
     */
    public static String getScope(Element topicref, String href) {
        ...
        if (scope == null) {
            if (href != null &&
                ...
                "true".equals(
                    DITAUtil.getNonEmptyAttribute(topicref, DITAC_NS_URI,
                                                  ABSOLUTE_HREF_NAME))) {
                scope = "external";
            } else {
                scope = "local";
    ...
    /**
     * Return the <tt>format</tt> of specified <tt>topicref</tt>. 
    ...
    /**
     * Returns the lower-case DITA format (dita, ditamap, ditaval, html, etc) 
            ...
            if (href.length() > 0) {
                ...
                if (ext != null) {
                    ...
                    if (href.startsWith("http://") ||
                        href.startsWith("https://")) {
                        // Example: "http://www.xmlmind.com/ditac/".
                        format = "html";
                    } else if (href.endsWith("/")) {
                        // Like in "application/x-directory".
                        format = "x-directory";
                    }
                }
            }
            // Otherwise, href is a pure fragment: cannot really guess the
            // format.
        }
    ...
    public static Element[] findDitavalrefs(Element element) {
        Element[] ditavalrefs = null;
        ...
        while (child != null) {
            if (child.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (DITAUtil.isDitavalref(childElement)) {
                    if (ditavalrefs == null) {
                        ditavalrefs = new Element[] { childElement };
                    } else {
                        ditavalrefs =
                            ArrayUtil.append(ditavalrefs, childElement);
                    }
                }
            }
    ...
    public static boolean isDitavalref(Element element) {
        return DITAUtil.hasClass(element, "ditavalref-d/ditavalref");
    }

    public static boolean containsDitavalrefs(Element element) {
        return (DITAUtil.findDescendantByClass(
                    element, 0, "ditavalref-d/ditavalref") != null);
    }
    ...
    public static final String quotePathSegment(String segment) {
