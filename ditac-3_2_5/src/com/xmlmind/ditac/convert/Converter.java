...
public class Converter {
    ...
    public int run(String[] args) {
        preProc = createPreProcessor(console);
    ...
    /**
     * Create and return a new {@link PreProcessor}.
     */
    protected PreProcessor createPreProcessor(Console console) {
        return new PreProcessor(console);
    }
    ...
    private void showMemoryUsage() {
        if (console.isShowing(Console.MessageType.DEBUG)) {
            ...
            console.debug(Msg.msg("memoryUsage",
                                  Long.valueOf(usedMemory/1048576),
                                  Long.valueOf(totalMemory/1048576),
                                  Long.valueOf(maxMemory/1048576)));
        }
    }
    ...
    private URL getStyleSheet(String styleSheetPath) {
        ...
        if (transformURL == null) {
            ...
            if (transformURL == null) {
                console.error(Msg.msg("xsltNotFound", styleSheetPath));
            }
        }
        return transformURL;
    }
    ...
    private boolean parseOptions(String[] args) {
        ...
        for (l = 0; l < args.length; ++l) {
            ...
            if ("-p".equals(arg) || "-param".equals(arg)) {
                ...
                styleSheetURL = urlOrFile(location);
    ...
    private static URL urlOrFile(String location) {
        URL url = null;

        // Useful for something like: "-t ditac-xsl:fo/fo_indent.xsl".
        String resolved = Resolve.resolveURI(location);
        if (resolved != null) {
            try {
                url = URLUtil.createURL(resolved);
            } catch (MalformedURLException ignored) {}
        } else {
            url = URLUtil.urlOrFile(location);
        }
        ...
        if (split.length > 0) {
            ...
            for (int i = 0; i < split.length; ++i) {
                ...
                for (int j = 0; j < split2.length; ++j) {
                    ...
                    } else if ("equationlist".equals(section)) {
                        item |= PreProcessor.EQUATION_LIST;
        ...
        for (File file : files) {
            ...
            if (baseName.startsWith("fop")) {
                ...
                if (exeFile.isDirectory()) {
                    // As of FOP 2.2, it's fop-N.M/fop/fop,fop.bat.
                    exeFile = new File(exeFile, "fop");
                }
