...
public final class XMLUtil {
    ...
    public static Reader createReader(InputStream in, String fallbackEncoding, 
        ...
        String charset = detectEncoding(bytes, byteCount, bomLength);
        if (bomLength[0] > 0) {
    ...
    /**
     * Detect XML encoding by examining specified bytes which 
     * have been read at the very start of an XML file.
     *
     * @param bytes bytes read at the very start of an XML file
     * @param byteCount number of bytes read at the very start of an XML file
    ...
    public static String detectEncoding(byte[] bytes, int byteCount, 
        ...
        if (byteCount > 0) {
            ...
            if (charset == null) {
                ...
                String text = null;
                try {
                    text = new String(bytes, 0, byteCount, "US-ASCII");
                } catch (UnsupportedEncodingException cannotHappen) {}

                if (text != null && text.startsWith("<?xml")) {
                    Pattern pattern = 
